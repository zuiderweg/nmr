...
! assumes that 6-10 ppm has been selectively excited
! assumes strong spinlock (5KHz) that keeps the range 6-10 ppm as "like spins"
! Written by Erik R.P. Zuiderweg, The University of Michigan, Ann Arbor, MI, USA
! and Radboud University, Nijmegen, The Netherlands
! March  2021 (zuiderwe@umich.edu or erikzuiderweg@gmail.com)
...
!!! following are spinlocked with the amides

	if(atname(2:3).eq.'HN'.or.atname(2:3).eq.'H ') then
	R2=R2+5.60e11/20.*(9*tc + 15*tc/(1.+(tc*wh)**2) + 6*tc/(1.+(tc*wh*2.)**2))/(distance**6)  ! like
		write(*,*) resname, resnumber, atname , distance, 'like'
	goto 2000
	endif

	if(resname.eq.'PHE') then
	if(atname(2:3).eq.'HD') then
	R2=R2+5.60e11/20.*(9*tc + 15*tc/(1.+(tc*wh)**2) + 6*tc/(1.+(tc*wh*2.)**2))/(distance**6)  ! like
		write(*,*) resname, resnumber, atname , distance, 'like'
	goto 2000
	endif
	...
	if(resname.eq.'ARG') then
	if(atname(1:2).eq.'HH') then
	R2=R2+5.60e11/20.*(9*tc + 15*tc/(1.+(tc*wh)**2) + 6*tc/(1.+(tc*wh*2.)**2))/(distance**6)  ! like
		write(*,*) resname, resnumber, atname , distance, 'like'
	goto 2000
	endif
	...
	R2=R2+5.60e11/20.*(5*tc_methyl + 9*tc_methyl/(1.+(tc_methyl*wh)**2) + 6*tc_methyl/(1.+(tc_methyl*wh*2.)**2))/(distance**6)  ! unlike , fast motion
		write(*,*) resname, resnumber, atname , distance, ' unlike fast '
	goto 2000
	endif
	endif
	...
	R2=R2+5.60e11/20.*(5*tc + 9*tc/(1.+(tc*wh)**2) + 6*tc/(1.+(tc*wh*2.)**2))/(distance**6)  ! unlike
		write(*,*) resname, resnumber, atname , distance, ' unlike '
	goto 2000
	endif
