! program to calculate selective and unslective R1 relaxation rates  of alpha protons (HA) from a pdb file
! allows another correlation time for HA-methyl relaxation vector motion
! Erik R.P. Zuiderweg, The Uiversity of Michigan, Ann Arbor MI USA, Radboud University, Nijmegen, The Netherlands
! March 2021
! Donot distribute without the author's consent.
	...
	real:: cutoff, distance, x_ref, y_ref, z_ref, R1, R1_SEL
...
!!! start program
 ...
 22	format(' file exists. overwrite?  (y/n) > ', $)
 	...
 	write(11,*)   ' resnumber atname R1 R1_SEL'
...
! MASTERLOOP over all protons in the pdb file
 ...
 2000	read(unit=10,fmt=2,err=2001, end=2001 )line
	...
	if(atname(1:1).eq.'H'.or.atname(2:2).eq.'H' ) then   ! BIG LOOP
	...
	R1=R1+5.60e11/20.*(6.*tc_methyl/(1.+(wh*tc_methyl)**2)+24.*tc_methyl/(1.+(2.*wh*tc_methyl)**2))/(distance**6)   ! unselective fast motion
	R1_SEL=R1_SEL+5.60e11/20.*(2.*tc_methyl+6.*tc_methyl/(1.+(wh*tc_methyl)**2)+12.*tc_methyl/(1.+(2.*wh*tc_methyl)**2))/(distance**6) ! selective, fast motion
	endif
	endif
	...
	R1=R1+5.60e11/20.*(6.*tc/(1.+(wh*tc)**2)+24.*tc/(1.+(2.*wh*tc)**2))/(distance**6)   ! unselective all others
	R1_SEL=R1_SEL+5.60e11/20.*(2.*tc+6.*tc/(1.+(wh*tc)**2)+12.*tc/(1.+(2.*wh*tc)**2))/(distance**6) ! selective, all others
	endif
...
!	wh=yh*B0
!	wn=yn*B0
! 	R1_HH_UNSELECTIVE=0.05d0*(mu*hbar*yh**2)**2
!	1	*(6.d0*tc/(1.d0+(wh*tc)**2)+24.d0*tc/(1.d0+(2.d0*wh*tc)**2))
! 	R1_HH_SELECTIVE=0.05d0*(mu*hbar*yh**2)**2
!	1	*(2*tc+6.d0*tc/(1.d0+(wh*tc)**2)+12.d0*tc/(1.d0+(2.d0*wh*tc)**2))

! parsing out the constants we get
! mu^2 yh^4 hbar^2 =5.60e-49
! angstrom correction 1e60
! R2 = 5.60e11 / 20 * ( 9tc + 15 tc/(1+wh2tc^2) + 6 tc/(1+4w^2tc^2))
! 	R1_HH_UNSELECTIVE=5.60e11 / 20.  *(6.d0*tc/(1.d0+(wh*tc)**2)+24.d0*tc/(1.d0+(2.d0*wh*tc)**2))
! 	R1_HH_SELECTIVE=5.60e11 / 20. *(2.*tc+6.d0*tc/(1.d0+(wh*tc)**2)+12.d0*tc/(1.d0+(2.d0*wh*tc)**2))

 	write(11,*)  resnumber_ref, atname_ref , R1, R1_SEL
